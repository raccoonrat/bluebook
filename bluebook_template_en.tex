...
Last, the unified R\&D framework is an \textbf{operating program}: a single artifact chain \textbf{theory $\rightarrow$ system $\rightarrow$ benchmark $\rightarrow$ release}, with one source of truth for joinable objects, one release gate, and one evolution cadence. Details appear in the appendix on the unified R\&D operating program. The durable asset is a system that continuously compiles risk semantics into control, deposits control into evidence, and compiles evidence into bounded guarantees—not a longer module list.
}
...
\begin{itemize}
  ...
  \item Axiomatic characterization, non-goals / out-of-scope architectures; boundary and threat table; CIA+ objectives.
...
\section{Axiomatic characterization of \ControlPlane{} and four-part closure}
\textbf{Main thread:} \ControlPlane{}. This technical report treats \ControlPlane{} as the boundary within which four primitives—\textbf{judgment, execution, evidence, release}—must share \texttt{policy\_version}, compatible event schemas, and join keys so that obligations remain mutually reconstructible under dynamics (Section~\ref{sec:why-control-plane-necessary}). Engineering accepts \ControlPlane{} where four-part closure is required; alternatives split identifiers or schemas and therefore cannot sustain bounded deployment contracts without reconciliation overhead.

\section{Non-goals and out-of-scope architectures}
\textbf{Main thread:} \ControlPlane{}. This section lists architectures \emph{explicitly outside} the guarantee-production thesis—not as dismissals but as scope boundaries:
\begin{itemize}
  \item \textbf{Model-centric platforms alone} optimize generation \textsc{sla} but do not bundle tenant/tool/retrieval/release lifecycle obligations under one regression surface unless augmented by this closure machinery.
  \item \textbf{Uncorrelated security features} (best-effort detectors, logs, \textsc{waf}s without shared event/evidence identifiers) optimize local KPIs while blocking coherent replay and cross-layer narratives for one request.
\end{itemize}
Only an enforced control-plane-shaped boundary requires identical \texttt{policy\_version} online and offline and yields differential replay and accountability slicing via unified evidence objects.

\noindent Responsibility-boundary topology is summarized in Appendix~\ref{fig:responsibility-topology}.

\section{Reading conventions and claim scope (summary)}
\textbf{Main thread:} \ControlPlane{} / \EvalFramework{}. We commit to \emph{deployable, measurable, and revocable} enterprise security contracts: under explicit premises (Chapter~5), bounded guarantees are provided through \DecoupledSafety{} and \EvalFramework{}. Concise non-goals appear above; full claim discipline, premises, and degradation regimes are in Chapter~5.

\section{Responsibility topology (summary)}
\textbf{Main thread:} \ControlPlane{}. Benchmark governance defines obligations and measurement conditions; the control plane owns judgment, routing, and policy evaluation; the execution environment owns execution, orchestration, and \textsc{sla} but does not interpret safety semantics; evidence/release gating elevates behavior into auditable release objects; jurisdictional and multilingual semantics enter policy and evidence scope as first-class intelligence (Chapter~6). See Appendix~\ref{fig:responsibility-topology}.
...
\textbf{Main thread:} \ControlPlane{}. The compositional grammar $G_{\mathrm{sys}}$ assumes rows append reliably with shared keys. Distributed realities impose explicit modes:
\begin{itemize}
...
\textbf{Main thread:} \ControlPlane{}. Chapter~3 fixes dual-plane correctness and budgets; here we instantiate triggers T1--T3 and actions D1--D4 under those budgets without repeating the architectural split.
...
S1 breach & B & B & B & B \\
Evidence join fail (mandatory row) & D/B & D/B & D/B & B \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent\textbf{Dual-timescale closure (cross-reference).} The cold-path compiler role of evaluation relative to the hot path is stated once in Chapter~3 (dual-plane correctness); Chapter~5 develops evidence packs; Figure~\ref{fig:dual-timescale-loop} summarizes the loop without repeating full methodology here.
...
\textbf{Main thread:} \EvalFramework{}. Treat each Release Evidence Pack as a \textbf{typed artifact}: every section must carry machine-checkable metadata (track, policy version, scope, approval pointers). Subjective narrative alone is inadmissible for Track~A.
...
The governance target is that no Track~A ship decision relies on packs containing unversioned blobs, mismatched \texttt{correlation\_id} samples, or Track~B-only proofs. Operationally, implement an automated \textbf{admissibility validator} in \textsc{ci}/release pipelines (hard failure on structural violations) rather than relying solely on manual vote---manual steps remain for risk acceptance, not for bypassing join closure.

\subsection{Cryptographic and systemic immutability (recommended baseline)}
\textbf{Main thread:} \EvalFramework{}. To resist retrospective tampering by compromised execution identities, \texttt{decision\_record\_id} issuance and evidence-plane appends should rely on an append-only or hash-chained store (or equivalent tamper-evident log service): each record binds predecessor hash, \texttt{policy\_version}, and partition key so reordering or deletion is detectable. Release gates reject packs whose cited rows fail integrity verification against the declared anchor.

\subsection{Ring-fencing external drift without policy bumps}
\textbf{Main thread:} \EvalFramework{} / \ControlPlane{}. Stateful tools and externally hosted retrieval indexes can shift behavior without a local \texttt{policy\_version} bump. Contracts must either (i)~pin endorsed tool endpoints, corpus generations, and embedding generations explicitly in the acceptance contract and evidence pack, or (ii)~treat unmanaged drift as Tier-B upper-bound evidence until re-validated under Track~A---preventing silent guarantee creep.
...
\textbf{Main thread:} Synthesis. Signable clauses covering scope, capability packs, S1 and degradation semantics, evidence obligations, engineering facts (config/logs/update), and change control---aligned with the Minimum Release Evidence Pack above.

\section{Evidence graph and release governance}
\textbf{Main thread:} \EvalFramework{} / \ControlPlane{}. Release decisions depend on an evidence graph connecting benchmark runs, policy versions, routing, detectors, index generations, telemetry slices, and approvals; differential reruns supported. Unified governance decision \(\neq\) unified score.
...
This appendix closes the artifact chain into an executable operating program: how theory becomes claimable properties, properties become system objects, system objects become benchmarks and evidence, and evidence becomes release decisions and bounded contracts. A unified program keeps one object language, one evidence gate, and one evolution cadence.

\section{Theory--system--benchmark--release: turning claims into a running pipeline}
The unified framework is not a parallel arrangement of ``research, engineering, evaluation, release,'' but a single traceable chain with feedback. \textbf{Theory} defines claim discipline, guarantee grammar, jurisdictional semantics, and claim boundaries; \textbf{System} compresses them into \texttt{event}, \texttt{decision}, \texttt{evidence}, and T/D/S/Tier-R/ORA objects; \textbf{Benchmark} compiles them into constitutions, acceptance contracts, multi-track suites, and evidence packs; \textbf{Release} consumes evidence at a unified governance board and outputs ship / ship reduced / canary / rollback as bounded deployment contracts.

The key constraint is: \textbf{any externally stateable conclusion must simultaneously cite theory boundaries, system objects, benchmark evidence, and release contract clauses}. Breaking this chain yields locally reassuring components without sustainable defensible guarantees.

\noindent\textbf{Compiler metaphor (assurance as artifact).} Theory plus obligation mappings act as ``source''; kernels, policies, and enforcement hooks as intermediate representations; benchmark outputs and evidence packs as typed artifacts; and the bounded deployment contract plus Release Evidence Pack as the shipped ``binary'' consumed by auditors and customers. Partial compilation produces unreleasable or misleading guarantees even when components ``feel'' secure.
...
\paragraph{Internal factorization (management note).} F1 semantics and obligation generation; F2 runtime control implementation; F3 measurement and evidence compilation; F4 release and supervision closure. Engineering anchors appear in the Controls-core subsection of Chapter~3.
