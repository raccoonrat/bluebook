...
\textbf{Main thread:} \ControlPlane{}. Chapter~3 fixes dual-plane correctness and budgets; here we instantiate triggers T1--T3 and actions D1--D4 under those budgets---only the tiered-timeout and S1 accounting specifics below extend that split.
...
\textbf{Hard accounting (informal).} Let $T_{\mathrm{budget}}$ denote the synchronous envelope for one request (e.g.\ gateway \textsc{p95}). A minimal decomposition is
\[
...
\end{figure}
\noindent Tracks share one execution backbone; governance convergence produces release decisions and audit artifacts without collapsing distinct track semantics into one aggregate score (see semantic bifurcation above).
...
\begin{figure}[htbp]
  ...
  \caption{Responsibility-boundary topology of the Unified AI Security Control Plane (referenced from Chapter~1; consolidated in the appendix for editorial flow)}
